...
export default function MenueMobile({close,open}:prop) {
  ...
  return (
        ...
        <CloseIcon className='absolute top-7 right-5 text-white z-10 text-3xl cursor-pointer' onClick={()=>close()}/>
            <Link to={"hero"}  smooth={true}  offset={-70}  className={`${styles.linkNav}`}>Home</Link>
            <Link to={"about"}  smooth={true}  offset={-70}  className={`${styles.linkNav}`}>About</Link>
            <Link to={"projects"}  smooth={true}  offset={-250}  className={`${styles.linkNav}`}>Projects</Link>
            <Link to={"contact"}  smooth={true}  offset={-70} className={`${styles.linkNav}`}>Contacts</Link>
        </div>
